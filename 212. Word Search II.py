        ...
        def getNeighbourCooridate(x,y):
            ...
            for direct in directions:
                ...
                if xd>=0 and xd<const_m and yd>=0 and yd<const_n  and visited[xd][yd]==0:# 这个要做一个某个字母是否已经用过了。
                    yield (xd,yd)
        def dfs(dict_data,prefix,pre_coor_list,first=False):
            if dict_data["$"]==True:
                ans_list.append(prefix)
                dict_data["$"]=False # 删除这个单词避免重复
                return True
            for xchar,son_dict_data in dict_data.items():
                if xchar=="$":
                    continue
                if first: # 存储上一个坐标留下来的可以选择的候选坐标
                    for (x,y) in coordinate_cache[xchar]:
                        new_pre_coor_list=list(getNeighbourCooridate(x,y))
                        visited[x][y]=1
                        res=dfs(son_dict_data,prefix+xchar, new_pre_coor_list)
                        visited[x][y]=0
                        if res:
                            break

                else:
                    # 要判断，当前的xchar对应的坐标和上一个字母邻接的坐标，是否有一样的。
                    for (x,y) in coordinate_cache[xchar]:
                        if (x,y) in pre_coor_list and visited[x][y]==0:
                            # 符合预期了。
                            new_pre_coor_list=list(getNeighbourCooridate(x,y))
                            if new_pre_coor_list:
                                visited[x][y]=1
                                res=dfs( son_dict_data,prefix+xchar, new_pre_coor_list)
                                visited[x][y]=0
                                if res:
